...
-(instancetype)initWithChalpter:(ChalpterDetailModel *)chalpter
{
    ...
    {
        self.readerPage = 0;
        self.textRenderRect = [CBReaderView textContentRect];
        // 章节模型
        self.chapter = chalpter;
    }
    return self;
}

-(void)setChapter:(ChalpterDetailModel *)chapter
{
    _chapter = chapter;
    
    // 初始化参数
    _contentText = chapter.cpContent;
    if (chapter.cpContent.length < 1) {
        _contentText = chapter.body;
    }
    _contentFont = [CBReaderStyle shareStyle].font.pointSize;
    // 分配页数
    [self paginate];
}
...
- (void)paginate
{
    ...
    CGPathRef path = CGPathCreateWithRect(_textRenderRect, NULL);
    ...
    while (hasMorePages)
    {
        ...
        else if ((range.location + range.length) == attrString.length &&
                 (currentOffset + range.length) != [_contentText length])
        {
            // 加载后面的
