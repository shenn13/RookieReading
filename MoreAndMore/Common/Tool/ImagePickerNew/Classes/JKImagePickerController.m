...
- (void)viewDidLoad {
    ...
    if (self.isXaingJi) {
        [self startPhotoAssetsViewCell:nil];
    }
...
- (void)showAssetsGroupView
{
    ...
    [UIView animateWithDuration:0.3f
                     animations:^{
                         self.assetsGroupsView.jk_top = 0;
...
- (void)hideAssetsGroupView
{
    [UIView animateWithDuration:0.3f
                     animations:^{
                         self.assetsGroupsView.jk_top = -self.assetsGroupsView.jk_height;
                         self.overlayView.alpha = 0.0f;
                     }completion:^(BOOL finished) {
...
- (void)setSelectAssetsGroup:(ALAssetsGroup *)selectAssetsGroup{
    if (_selectAssetsGroup != selectAssetsGroup) {
        ...
        self.titleButton.jk_width = width*2;
        
        self.titleLabel.jk_centerY = self.titleButton.jk_height/2;
        self.titleLabel.jk_centerX = self.titleButton.jk_width/2;
        
        self.arrowImageView.jk_left = self.titleLabel.jk_right + 5;
        self.arrowImageView.jk_centerY = self.titleLabel.jk_centerY;
...
{    
    ...
    if ([indexPath row]<=0)
    {
        UICollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:KJKImage forIndexPath:indexPath];
        if (!self.previewLayer) {
            self.previewLayer = [[AVCaptureVideoPreviewLayer alloc] initWithSession:self.session];
        }
        if (![cell.contentView viewWithTag:100]) {
            _photoButton = [UIButton buttonWithType:UIButtonTypeCustom];
            _photoButton.tag=100;
            _photoButton.frame = cell.contentView.bounds;
            ...
            [_photoButton addTarget:self action:@selector(startPhotoAssetsViewCell:) forControlEvents:UIControlEventTouchUpInside];
        ...
        cell.delegate = self;
        ALAsset *asset = self.assetsArray[self.assetsArray.count-(indexPath.row-1)-1];
        cell.asset = asset;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row==0) {
        ...
        {
            if ([subView isKindOfClass:[JKPhotoBrowser class]])
            {
                return;
            }
        }
        // 大图查看
        [self browerPhotoes:self.assetsArray page:self.assetsArray.count-(indexPath.row-1)-1];
    }
}
...
- (void)resetFinishFrame
{
    ...
    self.finishButton.jk_width = _finishLabel.jk_width+10;
    self.finishButton.jk_right = self.view.jk_width - 10;
    self.finishLabel.jk_centerX = self.finishButton.jk_width/2;
    self.finishLabel.jk_centerY = self.finishButton.jk_height/2;
...
- (JKAssetsGroupsView *)assetsGroupsView{
    if (!_assetsGroupsView) {
        _assetsGroupsView = [[JKAssetsGroupsView alloc] initWithFrame:CGRectMake(0, -self.view.jk_height, self.view.jk_width, self.view.jk_height)];
...
- (UIButton *)touchButton{
    if (!_touchButton) {
        ...
        _touchButton.frame = CGRectMake(0, 0, self.view.jk_width, 64);
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        _toolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.view.jk_height-44-64, self.view.jk_width, 44)];
        ...
        label.text = @"原图";
        [label sizeToFit];
        label.jk_centerY = _selectButton.jk_height/2;
        label.jk_right = _selectButton.jk_width - 14;
        ...
        _finishButton.frame = CGRectMake(0, 0, _finishLabel.jk_width+10, _selectButton.jk_height);
        ...
        _finishButton.jk_right = self.view.jk_width-10;
        _finishButton.jk_centerY = _selectButton.jk_centerY;
        ...
        _finishLabel.jk_centerY = _finishButton.jk_height/2;
        _finishLabel.jk_centerX = _finishButton.jk_width/2;
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.jk_width, [UIScreen mainScreen].bounds.size.height-64-44) collectionViewLayout:layout];
        _collectionView.backgroundColor = [UIColor clearColor];
        [_collectionView registerClass:[JKAssetsViewCell class] forCellWithReuseIdentifier:kJKImagePickerCellIdentifier];
        [_collectionView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:KJKImage];
